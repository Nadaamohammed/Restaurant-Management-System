...
public static class DeliveryUI
{
    ...
    private static void ListDeliveries()
    {
        ...
        foreach (var d in Database.Deliveries)
        {
            ...
            var color = d.Status switch {
                DeliveryStatus.Delivered => ConsoleColor.Green,
                DeliveryStatus.Failed    => ConsoleColor.DarkRed,
                DeliveryStatus.OnTheWay  => ConsoleColor.Yellow,
                _ => ConsoleColor.White
            };
            Console.ForegroundColor = color;
            Console.WriteLine($"   {d.DeliveryId,-6} {d.OrderId,-6} {d.DeliveryAddress,-28} {staff,-20} {d.Status,-22} {d.DeliveryTime?.ToString("HH:mm") ?? "—"}");
            Console.ResetColor();
    ...
    private static void MarkDelivered()
    {
        ...
        var active = Database.Deliveries.Where(d => d.Status == DeliveryStatus.OnTheWay).ToList();
